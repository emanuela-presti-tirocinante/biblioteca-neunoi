...
const AdminCatalog = () => {
    ...
    const [activeTab, setActiveTab] = useState(location.state?.tab || 'books');
            ...
            <div className="fixed inset-0 z-[200] flex flex-col justify-end p-4">
                ...
                <div className="relative bg-white rounded-3xl p-8 shadow-2xl animate-slide-up space-y-8 w-full max-w-lg mx-auto">
                    ...
                    <div className="flex flex-col space-y-3 pt-4">
                        ...
                        <button
                            onClick={() => setIsDeleteSheetOpen(false)}
                            className="w-full bg-white border border-gray-100 text-gray-400 py-5 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-gray-50 transition-all"
                        >
                            ANNULLA
                        </button>
                    </div>
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#F8F9FA] pb-20">
            ...
            <div className="bg-white border-b border-gray-100 px-4 py-4 flex overflow-x-auto no-scrollbar space-x-3 items-center sticky top-0 z-20">
                <button
                    ...
                    className={`relative overflow-visible px-6 py-2 rounded-[20px] text-[10px] font-black uppercase tracking-wider transition-all
                        ${activeTab === 'books' ? 'bg-secondary text-white shadow-lg' : 'bg-[#F2F2F2] text-gray-400 hover:bg-gray-200'}`}
                >
                    LIBRI
                    <span className="absolute top-0 right-0 translate-x-1/2 -translate-y-1/2 bg-yellow-400 text-gray-900 text-[9px] font-black w-5 h-5 flex items-center justify-center rounded-full shadow-md border-2 border-white z-10">
                        {stats.books}
                    </span>
                </button>
                <button
                    ...
                    className={`relative overflow-visible px-6 py-2 rounded-[20px] text-[10px] font-black uppercase tracking-wider transition-all
                        ${activeTab === 'categories' ? 'bg-secondary text-white shadow-lg' : 'bg-[#F2F2F2] text-gray-400 hover:bg-gray-200'}`}
                >
                    CATEGORIE
                    <span className="absolute top-0 right-0 translate-x-1/2 -translate-y-1/2 bg-yellow-400 text-gray-900 text-[9px] font-black w-5 h-5 flex items-center justify-center rounded-full shadow-md border-2 border-white z-10">
                        {stats.categories}
                    ...
                    <div className="flex flex-col space-y-4">
                        ...
                        {activeTab === 'books' ? (
                            <div className="flex flex-col space-y-2">
                                ...
                                {/* Etichetta categoria selezionata */}
                                {selectedCategory && (
                                    <div className="flex items-center space-x-2 px-1">
                                        <span className="text-[9px] font-black text-primary uppercase tracking-widest">Categoria:</span>
                                        <span className="text-[9px] font-black bg-accent text-primary px-2 py-0.5 rounded uppercase">
                    ...
                    {activeTab === 'books' && totalPages > 1 && (() => {
                        ...
                        return (
                            <div className="flex justify-center items-center gap-1.5 pt-4 pb-8">
                                <button onClick={() => fetchBooks(page - 1)} disabled={page === 1} className={`${btnBase} ${btnArrow} disabled:opacity-30 disabled:cursor-not-allowed`}>
                                ...
                                {buildPages().map((p, i) =>
                                    p === '...' ? (
                                        ...
                                        <button key={p} onClick={() => fetchBooks(p)} className={`${btnBase} ${p === page ? btnActive : btnDefault}`}>{p}</button>
                                    )
                                )}
                                <button onClick={() => fetchBooks(page + 1)} disabled={page === totalPages} className={`${btnBase} ${btnArrow} disabled:opacity-30 disabled:cursor-not-allowed`}>
                                    <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {/* Back to top button */}
            <button onClick={scrollToTop} className="fixed bottom-6 right-6 w-12 h-12 bg-primary text-white rounded-full flex items-center justify-center shadow-lg active:scale-95 transition-all z-40 border-2 border-accent">
